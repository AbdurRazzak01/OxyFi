...
export const AppBar: React.FC = () => {
  ...
  return (
    <div className="sticky top-0 z-50">
      {/* NavBar / Header */}
      <div className="navbar flex h-16 md:h-20 flex-row shadow-lg bg-white dark:bg-black text-gray-900 dark:text-neutral-content border-b border-gray-200 dark:border-zinc-600 bg-opacity-95 dark:bg-opacity-95 backdrop-blur-sm transition-all duration-200">
        <div className="navbar-start flex items-center">
          {/* Logo - Hidden on very small screens, shown on sm+ */}
          <div className="hidden sm:flex w-auto h-auto md:p-2 ml-4 md:ml-10">
            <Link href="https://solana.com" target="_blank" rel="noopener noreferrer" passHref className="text-gray-600 dark:text-secondary hover:text-purple-600 dark:hover:text-white transition-colors duration-200">
              <svg width="120" height="20" viewBox="0 0 646 96" fill="none" xmlns="http://www.w3.org/2000/svg" className="hover:scale-105 transition-transform duration-200">
          ...
          <div className="flex md:hidden ml-4">
            <WalletMultiButtonDynamic className="btn-ghost btn-sm text-sm bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white border-none transition-all duration-200 hover:scale-105" />
          </div>
        </div>
        ...
        {/* Navbar End */}
        <div className="navbar-end flex items-center space-x-2 md:space-x-4 mr-4 md:mr-6">
          
          {/* Desktop controls */}
          <div className="hidden md:flex items-center space-x-4">
            <DarkModeToggle />
            <WalletMultiButtonDynamic className="btn-ghost btn-sm rounded-btn text-sm bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white border-none transition-all duration-200 hover:scale-105" />
          </div>

          {/* Settings dropdown */}
          <div className="dropdown dropdown-end">
            <div tabIndex={0} className="btn btn-square btn-ghost text-gray-700 dark:text-gray-300 hover:text-purple-600 dark:hover:text-purple-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-200 hover:scale-105">
            ...
            <ul tabIndex={0} className="p-3 shadow-lg menu dropdown-content bg-white dark:bg-gray-800 rounded-box w-52 border border-gray-200 dark:border-gray-700 animate-slide-down">
              <li>
                <div className="form-control">
                  <label className="cursor-pointer label justify-between">
                    <span className="label-text text-gray-700 dark:text-gray-300">Autoconnect</span>
                    <input 
                      type="checkbox" 
                      checked={autoConnect} 
          ...
          <button
            className="btn btn-square btn-ghost md:hidden text-gray-700 dark:text-gray-300 hover:text-purple-600 dark:hover:text-purple-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-200"
            onClick={() => setIsNavOpen(!isNavOpen)}
            aria-label="Toggle navigation menu"
          >
            <div className="w-6 h-6 flex flex-col justify-center items-center">
              <div className={`w-5 h-0.5 bg-current transition-all duration-300 ${isNavOpen ? 'rotate-45 translate-y-0.5' : ''}`} />
              <div className={`w-5 h-0.5 bg-current transition-all duration-300 mt-1 ${isNavOpen ? 'opacity-0' : ''}`} />
              <div className={`w-5 h-0.5 bg-current transition-all duration-300 mt-1 ${isNavOpen ? '-rotate-45 -translate-y-1.5' : ''}`} />
            </div>
          </button>
        </div>
      </div>
      ...
      <div className={`md:hidden bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 transition-all duration-300 ease-in-out ${
        isNavOpen ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0 overflow-hidden'
      }`}>
